...
(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    var bd = t.closest && t.closest('[data-badge-toggle]');
    if (bd) {
      e.preventDefault();
      var bi = parseInt(bd.getAttribute('data-badge-toggle'), 10);
      var badge = lastBadges[bi];
      var pop = el('badgePop');
      if (!badge || !pop) return;
      var row = bd.parentNode;
      var already = bd.getAttribute('aria-expanded') === 'true';
      if (row) {
        var chips = row.querySelectorAll('[data-badge-toggle]');
        for (var bj = 0; bj < chips.length; bj++) chips[bj].setAttribute('aria-expanded', 'false');
      }
      if (already) { pop.hidden = true; pop.innerHTML = ''; return; }
      bd.setAttribute('aria-expanded', 'true');
      pop.innerHTML = '<b>' + esc(badge.name) + '</b> ' + badge.blurb;
      pop.hidden = false;
      return;
    }
  ...
  function buildCtx() {
    ...
    return {
      ...
      /* strength-aware gate: like `is`, but returns a weight scaled by how
       * firmly the option was held — conviction 1.0, a bare lean 0.55, and
       * ×0.8 again if hedged with a "but" (so a hedged lean is only ~0.44).
       * 0 when it isn't a pick. Niche archetypes and badges gate on THIS, not
       * on `is`, so a tentative answer can't capture the result the way a
       * conviction can — the whole point of "make them harder / more sure". */
      g: function (id, idx) {
        var a = answers[id];
        if (!a || a.strength === 'agnostic' || a.strength === 'nofact' || a.strength === 'other' || !isPick(id, idx)) return 0;
        var w = a.strength === 'firm' ? W_FIRM : W_LEAN;
  ...
  /* Every pick-based bonus below is scaled by `c.g` (strength-aware): a
   * conviction counts full, a bare lean ~0.55, a hedged lean ~0.44. So the
   * archetypes are "harder / more sure" — a tentative answer no longer
   * captures the result. The four thinnest single-pick types (Modal Realist,
   * Dialetheist, Mysterian, Moral Particularist) have been moved down to
   * BADGES, which decorate whichever headline you land on. */
  var ARCHETYPES = [
    { name: 'The Naturalist',
      ...
      score: function (c) { var s = 1.0 * c.phys + 0.8 * c.emp + 0.9 * c.nat + 0.3 * c.cons; s += 0.4 * c.g(59, 1) + 0.3 * c.g(15, 0) + 0.2 * c.g(16, 0); return s; } },
    { name: 'The Utilitarian',
      ...
      score: function (c) {
        ...
        s += 0.6 * c.share(20, 1) + 0.3 * c.g(28, 0) + 0.4 * c.g(34, 0);
        return s; } },
    { name: 'The Platonic Rationalist',
      ...
      score: function (c) { var s = 1.0 * c.rat + 1.0 * c.plat + 0.5 * c.real; s += 0.4 * c.g(2, 0) + 0.2 * c.g(1, 0); return s; } },
    { name: 'The Kantian',
      ...
      score: function (c) { var s = 0.9 * c.real + 0.9 * c.deont + 0.5 * c.rat; s += 0.6 * c.g(82, 2) + 0.5 * c.share(20, 0) + 0.3 * c.g(15, 1); return s; } },
    { name: 'The Theistic Traditionalist',
      ...
      score: function (c) { var s = 1.2 * c.theist + 0.6 * c.real + 0.4 * c.deont; s += 0.6 * c.g(8, 0); return s; } },
    { name: 'The Aristotelian',
      ...
      score: function (c) { var s = 0.5 * c.real; s += 1.3 * c.share(20, 2) + 0.6 * c.g(82, 0) + 0.3 * (c.share(31, 2) + c.share(31, 3)) + 0.3 * c.g(71, 1); return s; } },
    { name: 'The Existentialist',
      ...
      score: function (c) { var s = 0.9 * (-c.real) + 0.3 * c.nat; s += 0.6 * c.g(36, 0) + 0.4 * c.g(36, 2) + 0.3 * c.g(3, 1) + 0.2 * c.g(33, 1); return s; } },
    { name: 'The Pragmatist',
      ...
      score: function (c) { var s = 0.3 * c.nat + 1.1 * c.nofactFrac; s += 0.6 * c.g(29, 1); if (c.multiCount(37) >= 3) s += 0.6; s += 0.3 * c.g(100, 1) + 0.2 * c.g(61, 1) + 0.3 * c.g(67, 1); return s; } },
    { name: 'The Idealist',
      ...
      score: function (c) { var s = 0.9 * (-c.phys); s += 0.6 * c.g(6, 0) + 0.3 * c.g(16, 1) + 0.4 * Math.max(c.g(50, 0), c.g(50, 4)) + 0.2 * c.g(59, 0); return s; } },
    { name: 'The Egalitarian Humanist',
      ...
      score: function (c) { var s = 1.0 * c.egal + 0.4 * c.real; s += 0.4 * c.g(78, 1) + 0.4 * c.g(23, 1) + 0.2 * c.g(53, 1); return s; } },
    ...
    { name: 'The Stoic',
      ...
      score: function (c) { var s = 0.9 * c.share(20, 2) + 0.3 * c.nat; s += 0.3 * c.g(82, 0) + 0.3 * c.g(33, 1) + 0.3 * c.g(62, 1) + 0.3 * Math.max(c.g(7, 0), c.g(7, 2)); return s; } },
    ...
    { name: 'The Hedonist',
      ...
      score: function (c) { var s = 0.4 * c.cons; s += 1.2 * c.g(99, 0) + 0.5 * c.g(33, 0) + 0.3 * c.share(20, 1); return s; } },
    ...
    { name: 'The Eliminativist',
      ...
      score: function (c) { var s = 0.8 * c.phys; s += 1.0 * c.g(50, 1) + 0.5 * c.g(83, 3) + 0.4 * c.g(59, 1) + 0.3 * c.g(30, 0); return s; } },
    ...
    { name: 'The Panpsychist',
      ...
      score: function (c) { var s = 0.5 * (-c.phys); s += 1.3 * c.g(50, 4) + 0.3 * c.g(59, 0) + 0.2 * c.g(58, 3); return s; } },
    ...
    { name: 'The Cartesian Dualist',
      ...
      /* tightened: "dualism" (q50, ~22%) is also picked by Idealists and
       * panpsychist-leaners, so it no longer carries this on its own — the
       * zombie / further-fact / hard-problem corroborators have to show up. */
      score: function (c) { var s = 0.6 * (-c.phys); s += 0.8 * c.g(50, 0) + 0.4 * c.g(22, 2) + 0.3 * c.g(30, 2) + 0.3 * c.g(59, 0) + 0.2 * c.g(26, 1); return s; } },
    ...
    { name: 'The Nominalist',
      ...
      /* tightened: a lone "nominalism" pick on q2 is a ~42% answer, so it can't
       * carry the type any more — it leans on the sparse-ontology axis plus the
       * genuinely distinctive nonexistent-properties / nonexistent-worlds picks. */
      score: function (c) { var s = 0.9 * (-c.plat); s += 0.45 * c.g(2, 1) + 0.6 * c.g(81, 4) + 0.5 * c.g(79, 2) + 0.3 * c.g(66, 0) + 0.3 * c.g(67, 2); return s; } },
    ...
    { name: 'The Libertarian',
      ...
      score: function (c) { var s = 0.7 * (-c.egal); s += 0.7 * c.g(23, 2) + 0.4 * c.g(78, 0) + 0.2 * c.g(65, 0); return s; } },
    ...
    { name: 'The Communitarian',
      ...
      /* tightened: the single q23 "communitarianism" pick (~27%) no longer
       * dominates; the political axis and the corroborating picks carry it. */
      score: function (c) { var s = 0.2 * c.real; s += 0.8 * c.g(23, 0) + 0.4 * c.g(68, 1); return s; } },
    ...
    { name: 'The Scientific Realist',
      ...
      score: function (c) { var s = 0.45 * c.phys + 0.45 * c.emp + 0.3 * c.nat; s += 0.5 * c.g(25, 0) + 0.5 * c.g(11, 0) + 0.3 * c.g(97, 0); return s; } },
    ...
    { name: 'The Instrumentalist',
      ...
      score: function (c) { var s = 0.4 * c.emp; s += 1.2 * c.g(25, 1) + 0.2 * c.g(11, 0) + 0.2 * c.g(29, 1); return s; } },
    ...
    { name: 'The Transhumanist',
      ...
      score: function (c) { var s = 0.7 * c.g(70, 0) + 0.7 * c.g(26, 0) + 0.4 * c.g(50, 2) + 0.3 * c.g(62, 0) + 0.2 * c.g(60, 0); return s; } },
    ...
    { name: 'The Sentientist',
      ...
      score: function (c) { var s = 0.4 * c.cons; s += 0.8 * c.g(32, 2) + 0.5 * c.g(32, 1) + 0.6 * c.g(53, 1) + 0.3 * c.g(74, 2) + 0.2 * c.g(74, 4); return s; } },
    ...
    { name: 'The Humean',
      ...
      score: function (c) { var s = 0.4 * c.emp; s += 0.7 * c.g(11, 0) + 0.5 * c.g(82, 1) + 0.3 * c.g(61, 1) + 0.3 * c.g(80, 1) + 0.2 * c.g(47, 0) + 0.3 * c.g(47, 3); return s; } },
    ...
    { name: 'The Phenomenologist',
      ...
      score: function (c) { var s = 0.2 * (-c.phys); s += 1.0 * c.g(37, 7) + 0.4 * c.g(15, 1) + 0.2 * c.g(59, 0); return s; } },
    ...
    { name: 'The Conceptual Analyst',
      ...
      score: function (c) { var s = 0.4 * c.g(37, 0) + 0.35 * c.g(37, 4); s += 0.35 * c.g(1, 0) + 0.3 * c.g(4, 0) + 0.2 * c.g(12, 0) + 0.2 * c.g(43, 0); return s; } },
    ...
    { name: 'The Critical Theorist',
      ...
      score: function (c) { var s = 0.2 * (-c.real) + 0.2 * c.egal; s += 0.7 * c.g(37, 10) + 0.6 * c.g(37, 12) + 0.4 * c.g(98, 1) + 0.3 * c.g(89, 2) + 0.2 * c.g(78, 1); return s; } },
    ...
    { name: 'The Error Theorist',
      ...
      score: function (c) { var s = 0.5 * (-c.real); s += 1.3 * c.g(72, 4) + 0.4 * c.g(17, 0) + 0.3 * c.g(14, 1); return s; } },
    ...
    { name: 'The Expressivist',
      ...
      score: function (c) { var s = 0.3 * (-c.real); s += 1.3 * c.g(72, 3) + 0.5 * c.g(17, 1); return s; } },
    ...
    { name: 'The Reductionist',
      ...
      score: function (c) { var s = 0.6 * c.g(26, 0) + 0.5 * c.g(70, 0) + 0.5 * c.g(22, 1) + 0.4 * c.g(7, 2) - 0.6 * c.g(22, 2); return s; } }
  ];

  /* ---- BADGES: distinctive single positions that decorate whatever headline
   * archetype you land on, rather than competing to BE the headline. Each was
   * previously a niche archetype that, on inspection, was really one rare
   * checkbox dressed up as a worldview — too thin to headline, too fun to drop.
   * `test` gates on `c.g` so only a real commitment (conviction, or a solid
   * un-hedged lean ≥ 0.5) earns the badge; a hedged lean does not. The blurb
   * survives as the click-to-expand popup. ---- */
  var BADGES = [
    { name: 'Modal Realist',
      ...
      test: function (c) { return c.g(79, 1) >= 0.5; } },
    { name: 'Dialetheist',
      ...
      test: function (c) { return c.g(96, 2) >= 0.5; } },
    { name: 'Mysterian',
      ...
      /* the McGinn signature needs BOTH halves: a real hard problem AND zombies
       * that are conceivable but not metaphysically possible (i.e. physicalism). */
      test: function (c) { return c.g(59, 0) >= 0.5 && c.g(30, 1) >= 0.5; } },
    { name: 'Moral Particularist',
      ...
      test: function (c) { return c.g(71, 1) >= 0.5; } }
  ];

  function computeBadges(c) {
    return BADGES.filter(function (b) { return b.test(c); });
  }
  ...
  /* the earned badges from the most recent render, so the delegated click
   * handler can pull a badge's blurb without re-running the scoring. */
  var lastBadges = [];

  /* a row of badge chips + a shared popover the chips open. Each chip is a
   * button; clicking it shows that badge's blurb below the row (clicking the
   * same chip again, or another, toggles/switches it). */
  function renderBadges(badges) {
    return '<div class="badges">' +
      '<span class="badges-label">Also</span>' +
      badges.map(function (b, i) {
        return '<button type="button" class="badge" data-badge-toggle="' + i + '" aria-expanded="false">✦ ' + esc(b.name) + '</button>';
      }).join('') +
      '</div>' +
      '<div class="badge-pop" id="badgePop" hidden></div>';
  }
  ...
  function renderResults() {
    ...
    var badges = arch.name === 'The Sphinx' ? [] : computeBadges(c);
    lastBadges = badges;
